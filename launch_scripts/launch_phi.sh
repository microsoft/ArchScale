...
for depth in 8; do
  for wp in 0; do
    for lr in 4e-3 8e-3 1e-2 1.2e-2 1.4e-2 1.6e-2 2e-2; do
      for aux_gamma in 1e-1; do
        for wd in 0; do
          for tokens in null; do
            for bdiv in 1; do
              wandb_project="muonh-ablation"
              fp8=false
              fsdp2=true
              aux_style="switch"
              bsz=2097152
              bsz0=2097152
              min_lr_mult=0.1
              train_model="transformer_gqa4_h2"
              act_ckpt=false
              if [[ ${depth} -ge 20 ]]; then
                act_ckpt=true
              fi
              ...
              train_name="v2scale_mup_muonh_bsz${bsz}_lr${lr}x${min_lr_mult}_wd${wd}_tok${tokens}_ga_qknorm_postnorm_decouple"
              PYTORCH_CUDA_ALLOC_CONF='expandable_segments:True' LIGHTNING_ARTIFACTS_DIR=${remote_output_dir} torchrun --nnodes=${nnodes} \
                  --nproc_per_node=${GPUS_PER_NODE} --node-rank=$NODE_RANK --rdzv-endpoint=${MASTER_ADDR} pretrain.py \
                    ...
                    --base_hps.eta0=${lr} --base_hps.b0=${bsz0} --base_hps.warmup_tokens=${wp} --base_hps.weight_decay=${wd} --total_bsz=${bsz} \
                    ...
                    --sparsity=32 --top_k=4 --num_extrp=${num_extrp} --fsdp2=${fsdp2} --act_ckpt=${act_ckpt} --offloading=${offloading} \
