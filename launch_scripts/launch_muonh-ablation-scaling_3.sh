...
for depth in 8 12 16 20; do
  for wp in 0; do
    for lr in 1.6e-2; do
      for aux_gamma in 1e-3; do
        for wd in 0; do
          for tokens in null; do
            for bdiv in 1; do
              wandb_project="muonh-ablation-scaling"
              fp8=false
              fsdp2=true
              aux_style="switch"
              bsz=2097152
              ...
              train_name="v2scale_mup_muonh_slr_bsz${bsz}_lr${lr}x${min_lr_mult}_wd${wd}_tok${tokens}_ga_qknorm"
