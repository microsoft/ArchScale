...
for depth in 24; do
  for wp in 0; do
    for tokens in 1e8; do
      for aux_gamma in 1e-1; do
        for wd in 0; do
          for lr in 1.2e-2; do
            for sparsity in 8; do
              for topk in 16 8 4 2; do
                share_expert=true
                global_aux=true
                sqrt_gate=true
                post_norm=null
                wandb_project="muonh-moe-k-scaling-mfu"
                fp8=false
                fsdp2=true
                aux_style="switch"
                min_lr_mult=0.1
                ...
                offloading=false
                base_tokens=1e8 # 50 TPP for chinchilla considering embedding parameters
                train_name="v2scale_mup_muonh_lr${lr}x${min_lr_mult}_wd${wd}_tok${tokens}_gblaux${aux_gamma}_ga_qknorm_sgate_shexp_test_mfu"
