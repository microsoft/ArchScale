...
def fit_optimal_loss(lrs, losses):
    """Fit quadratic in log-LR space and return the predicted loss at the optimum."""
    log_lrs = np.log(lrs)
    coeffs = np.polyfit(log_lrs, losses, 2)
    if coeffs[0] <= 0:
        return np.nan
    opt_log_lr = -coeffs[1] / (2 * coeffs[0])
    opt_loss = np.polyval(coeffs, opt_log_lr)
    return opt_loss

# --- Figure 1: Optimal Loss distribution vs number of fitting points ---
...
for ax, tok in zip(axes1, token_budgets):
    ...
    full_opt = fit_optimal_loss(all_lrs, all_losses)
    ...
    for k in ks:
        ...
        for combo in combinations(range(n), k):
            idx = list(combo)
            opt = fit_optimal_loss(all_lrs[idx], all_losses[idx])
            if not np.isnan(opt):
                opt_losses_k.append(opt)
    ...
    ax.fill_between(ks_arr, mins, maxs, alpha=0.15, color="darkorange", label="min-max")
    ax.fill_between(ks_arr, q25s, q75s, alpha=0.35, color="darkorange", label="25-75%")
    ax.plot(ks_arr, medians, "o-", color="darkorange", markersize=5, linewidth=1.5, label="Median")
    ...
    ax.set_ylabel("Fitted optimal Loss", fontsize=12)
    ax.set_xticks(ks_arr)
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig("loss_npoints_sensitivity.png", dpi=150, bbox_inches="tight")
print("Saved loss_npoints_sensitivity.png")
...
ax_re.set_xlabel("# LR data points used", fontsize=13)
ax_re.set_ylabel("Mean |Relative Error| to full fit (%)", fontsize=13)
ax_re.legend(fontsize=10, title="Token Budget")
ax_re.grid(True, alpha=0.3)
ax_re.set_yscale("log")
ax_re.xaxis.set_major_locator(plt.MaxNLocator(integer=True))

plt.tight_layout()
plt.savefig("loss_npoints_cv_error.png", dpi=150, bbox_inches="tight")
print("Saved loss_npoints_cv_error.png")

# --- Figure 3: Impact on downstream power-law fit (Loss vs tokens) ---
...
for tok in token_budgets:
    subset = data[data["tokens"] == tok].sort_values("lr")
    full_opt_losses.append(fit_optimal_loss(subset["lr"].values, subset["val_loss"].values))

try:
    popt_full, _ = curve_fit(power_law, np.array(token_budgets), np.array(full_opt_losses), p0=[10.0, -0.05])
except RuntimeError:
...
for k, col in zip(k_values, pl_colors):
    for _ in range(n_bootstrap):
        ...
        for tok in token_budgets:
            ...
            opt = fit_optimal_loss(lrs_arr[idx], losses_arr[idx])
            sampled_opt_losses.append(opt)

        if any(np.isnan(v) for v in sampled_opt_losses):
            continue

        try:
            popt_k, _ = curve_fit(power_law, np.array(token_budgets),
                                  np.array(sampled_opt_losses), p0=[10.0, -0.05])
...
ax_l.scatter(token_budgets, full_opt_losses, s=100, color="black", edgecolors="black",
             zorder=10, label="Full-data optimal Losses")
...
ax_l.set_xscale("log")
ax_l.set_yscale("log")
ax_l.set_xlabel("Training Tokens", fontsize=13)
ax_l.set_ylabel("Optimal Loss", fontsize=13)
...
plt.tight_layout()
plt.savefig("loss_npoints_powerlaw_impact.png", dpi=150, bbox_inches="tight")
print("Saved loss_npoints_powerlaw_impact.png")

# --- Print summary ---
print("\n" + "=" * 70)
print("=== Summary: Optimal Loss Sensitivity to # of LR Sweep Points ===")
print("=" * 70)
...
for tok in token_budgets:
    ...
    print(f"\n  Token budget: {tok/1e9:.1f}B  ({n} LRs)  |  Full-data optimal Loss: {full_opt:.6f}")
    print(f"  {'k':>4s}  {'#combos':>8s}  {'median':>10s}  {'mean':>10s}  {'std':>10s}  {'CV%':>8s}  {'relErr%':>8s}")
    ...
    for k in range(3, n + 1):
        ...
        if opt_losses_k:
            ...
            print(f"  {k:>4d}  {len(opt_losses_k):>8d}  {np.median(arr):>10.6f}  {np.mean(arr):>10.6f}  "
                  f"{np.std(arr):>10.6f}  {cv:>7.2f}%  {rel_err:>7.2f}%")
