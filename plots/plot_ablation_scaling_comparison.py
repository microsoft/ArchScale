"""
Compare dense model scaling curves (loss vs FLOPs) across ablation configs.
  - GatedAttn+QKNorm  (ga_qknorm, LR=1.6e-2)
  - QKNorm only       (qknorm,    LR=1.5e-2)
  - Baseline           (baseline,  LR=1.5e-2)

Data: muonh-ablation-scaling.csv  (W&B panel export, x-axis = total_tokens)
FLOPs: same dense formula as plot_scaling_comparison.py
...
df = pd.read_csv("muonh-ablation-scaling.csv")
...
def classify_method(name):
    if "ga_qknorm" in name:
        return "GatedAttn+QKNorm"
    elif "qknorm" in name:
        return "QKNorm"
    return "Baseline"
...
for col in loss_cols:
    ...
    vals = df[["total_tokens", col]].copy()
    vals["total_tokens"] = pd.to_numeric(vals["total_tokens"], errors="coerce")
    ...
    total_tokens = int(vals["total_tokens"].iloc[-1])
...
SERIES_ORDER = [
    ("Baseline",          0.015),
    ("QKNorm",           0.015),
    ("GatedAttn+QKNorm", 0.016),
]

COLORS = {
    ("Baseline",          0.015): "#e74c3c",
    ("QKNorm",           0.015): "#2ecc71",
    ("GatedAttn+QKNorm", 0.016): "#3498db",
}

MARKERS = {
    ("Baseline",          0.015): "s",
    ("QKNorm",           0.015): "D",
    ("GatedAttn+QKNorm", 0.016): "o",
}
...
fig.tight_layout()
fig.savefig("ablation_scaling_comparison.png", dpi=300, bbox_inches="tight")
print("\nSaved to ablation_scaling_comparison.png")
