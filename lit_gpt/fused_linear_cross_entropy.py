...
def fused_linear_cross_entropy_forward(
    ...
    # need to upcast to fp32 for numerical stability for large vocab size (200K)
    weight_fp32 = weight.float()
    _input_fp32 = _input.float()
    inc_factor = 8 #triton.cdiv(V, H)  # (V + H - 1) // H
    ...
    grad_weight = (
        torch.zeros_like(weight, device=device, dtype= torch.float32) if weight.requires_grad else None
    )
    grad_input = torch.zeros_like(_input, device=device, dtype= torch.float32)
    grad_bias = torch.zeros_like(bias, device=device, dtype= torch.float32) if bias is not None else None
    ...
    for chunk_id in range(num_chunks):
        ...
        _input_chunk_fp32 = _input_fp32[start_idx:end_idx]
        ...
        grad_input[start_idx:end_idx] = grad_logits_chunk @ weight_fp32

        if grad_weight is not None:
            torch.addmm(
                input=grad_weight,
                mat1=logits_chunk.t(),
                mat2=_input_chunk_fp32,
                out=grad_weight,
                alpha=alpha,
                beta=1.0,
            )
...
class FusedLinearCrossEntropyLoss(torch.nn.Module):
    ...
    def forward(self, input, weight, target, bias=None):
